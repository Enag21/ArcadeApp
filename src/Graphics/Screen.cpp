		...
		for(int pixelY = top; pixelY < bottom; ++pixelY)
		{
			...
			for(size_t k = 0; k < nodeXVec.size(); k+=2)
			{
				...
				if(nodeXVec[k+1] > left)
				{
					...
					//Line2D line = {Vector2D(nodeXVec[k], pixelY), Vector2D(nodeXVec[k+1], pixelY)};
					//Draw(line, color);
...
void Screen::DrawBoard(size_t tilesize)
{
	...
	for (size_t i = 0; i < 8; ++i)
	{
		...
		{
			Vector2D topLeft{10 + tilesize * j, 10 + tilesize * i};
			Vector2D delta{tilesize, tilesize};
			tiles.push_back(AARectangle{ topLeft, topLeft + delta});
		}
	}

	for(const auto& tile : tiles)
	{
		Draw(tile, Color::White());
	}
}
